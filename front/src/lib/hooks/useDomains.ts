...
export function useAddDomain() {
  ...
  return useMutation({
    mutationFn: async ({ siteId, data }: { siteId: string; data: CreateDomainData }) => {
      return apiClient.post(`/api/sites/${siteId}/domains`, data);
    },
    onSuccess: (_, variables) => {
      // Invalidar cache dos domínios do site
      queryClient.invalidateQueries({ queryKey: queryKeys.domains.all(variables.siteId) });
...
export function useRemoveDomain() {
  ...
  return useMutation({
    mutationFn: async ({ siteId, domainId }: { siteId: string; domainId: string }) => {
